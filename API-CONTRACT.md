...
- **`model` 예시가 `claude-haiku-4-5-20251001` 로 적혀 있었다 — D-024 확정값은 `claude-sonnet-5` 다.** 이 값은 단순 예시가 아니라 **측정 결과에 붙는 조건**이다. D-024 가 sonnet 을 고른 이유가 *"상위 모델을 쓰면 측정 8ⓐ 가 재려는 「AI 가 자신 있게 틀리는」 표본이 귀해진다"* 였는데, 계약대로 하위 티어를 쓰면 **그 판단을 아무도 다시 하지 않은 채 정확도만 달라진다.** 실제로 응답에 나가는 값은 설정값이 아니라 **AI 가 응답에서 말한 모델 id** 다
...
- `verdict = FAILED` 는 AI 호출·파싱 실패뿐 아니라 **값 검증 실패**도 포함한다 (D-034) — `confidence` 가 `0.0~1.0` 밖이거나, `category` 가 enum 10종에 없거나, 두 필드 중 하나가 아예 없는 경우다. **범위 밖 값을 잘라 넣지 않고 `FAILED` 로 본다** — `1.5` 를 `1.0` 으로 clamp 하면 측정 8ⓐ 의 최상위 구간이 오염된다
  ...
  - `FAILED` 인 건은 `category` 와 `confidence` 가 **둘 다 `null`** 이다. 하나만 `null` 인 조합은 나오지 않는다 — 한쪽만 비면 그건 `REUSED`(사람 확정 재사용)다
