...
| 500 | 그 외 처리되지 않은 서버 오류. `message` 는 고정 문구이고 내부 예외 메시지를 담지 않는다 | `INTERNAL_ERROR` |
...
> **저장되는 것은 원문이다.** 마스킹은 AI 로 보낼 때와 응답으로 내보낼 때 각각 계산하며, **두 자리가 같은 구현을 공유한다** (D-040). 두 벌로 두면 한쪽만 조여져 화면에는 가려지는데 프롬프트에는 남는 상태가 된다.
...
- `content` 는 **마스킹된 본문**을 반환한다. 원문은 저장하되 응답으로 되돌려주지 않는다
  - **마스킹본은 저장하지 않는다 (D-040).** 내보낼 때 계산한다 — 마스킹 규칙을 조이면 과거 문의까지 즉시 적용되어야 하기 때문이다. 저장해두면 백필을 빠뜨린 만큼 개인정보가 계속 나간다
- `confidence` 는 `ROLE_AGENT` 이상에게만 포함된다 — 고객에게는 생략
- **`confidence` 가 `null` 일 수 있다 (D-039).** `0` 이나 `"-"` 로 치환하지 않는다 — `0` 은 비교에서 **가장 낮은 신뢰도로 참여**해 조용히 틀린다

  | 표현 | 뜻 |
  | --- | --- |
  | 필드 자체가 없음 | **권한이 없어 주지 않는다** (`ROLE_CUSTOMER`) |
  | 필드가 있고 `null` | **값이 존재하지 않는다** — 미판정이거나, `FAILED` 이거나, **사람 확정을 재사용한 `REUSED`** (D-033) |

  `category` 가 `null` 인 것은 미판정과 `FAILED` 뿐이다. **재사용 건에는 카테고리가 있다.**
...
- `model` 에는 두 가지 중 하나가 들어간다
  - **실제 AI 호출** — AI 가 응답에서 말한 모델 id. 설정값이 아니다. 확정값은 `claude-sonnet-5` (D-024). **이 값은 예시가 아니라 측정 결과에 붙는 조건**이라, 다른 티어를 쓰면 D-024 의 선정 근거가 무너진다 (v1.5 에서 `claude-haiku-4-5-20251001` → 정정)
  - **재사용** — 원본 결과 id (D-031). 실제 호출과 구분되지 않으면 측정 6 과 8ⓑ 를 검산할 수 없다
- `verdict = REUSED` 는 AI 를 부르지 않고 같은 정규화 키의 원본 결과를 재사용한 건이다 (D-033). **사람이 확정한 답을 재사용했으면 `confidence` 가 `null`** 이다 — 사람은 확신도를 매기지 않으므로 `1` 이나 원본 AI 값을 채우지 않는다
  - 이때 목록(§2)의 `confidence` 와 `inquiries.current_confidence` 도 **`null` 이 그대로 내려간다** (D-039). 역정규화 사본은 요약이 아니라 사본이다. **스키마 변경은 없다** — `current_confidence` 는 `V2` 에서 이미 nullable 이다
- `verdict = FAILED` 는 **AI 를 못 부른 경우 + 답을 못 읽은 경우 + 값이 이상한 경우**를 함께 담는다 (D-034)
  ...
  **어떻게 하나** — 기준값과 비교하기 **전에** 네 가지를 본다. 하나라도 어긋나면 `FAILED` 다.

  | 검사 | 어긋난 예 |
  | --- | --- |
  | `confidence` 필드가 있는가 | 필드 없음 |
  | `0.0 ≤ confidence ≤ 1.0` 인가 | `1.5` · `-0.3` |
  | `category` 필드가 있는가 | 필드 없음 |
  | `category` 가 10종에 있는가 | `REFUND_XYZ` |

  **범위 밖 값을 잘라 넣지 않는다.** `1.5` 를 `1.0` 으로 clamp 하면 최상위 구간에 "AI 가 1.0 이라 신고한 건"과 "잘라낸 건"이 섞인다.

  **어떻게 확인하나** — `FAILED` 인 건은 `category` 와 `confidence` 가 **둘 다 `null`** 이다. 응답에서 이걸로 판별한다. 하나만 `null` 이면 그건 `FAILED` 가 아니라 `REUSED`(사람 확정 재사용)다.

  > **실패 사유 5종(`PARSE_ERROR` · `OUT_OF_RANGE` · `UNKNOWN_CATEGORY` · `MISSING_FIELD` · `API_ERROR`)은 응답에 나가지 않는다.** 구조화 로그로만 남는다 (TRI-50). 응답 필드로 두면 컬럼이나 enum 이 따라 늘고, 판정을 읽는 식이 `verdict` 하나가 아니게 된다 (계약 B · D-022). 사유별 건수는 측정 2 에서 로그를 세어 낸다.
