...
| 20건에 1건 감사가 잡아낸 몫 | **0.333** — 3건 중 1건. **2건은 놓쳤다**        | [`evidence/measurement-1-8a.md`](./evidence/measurement-1-8a.md) |
| 지난 답을 다시 쓴 건의 오답률 | **0.062** — 재사용이 오류를 **증폭하지 않았다**       | [`measurement-8b.md`](./evidence/measurement-8b.md) |
| AI 호출을 얼마나 아꼈나 | **0.490** (1000건 기준, 절감률) · 캐시 적중 0.454 | [`measurement-6-11-actual.md`](./evidence/measurement-6-11-actual.md) |
| 접수 응답 속도 (목표 0.1초) | **6.4~8.8초 — 목표를 못 넘었다** (측정 조건·표본은 evidence 참조)               | [`api-response-time.md`](./evidence/api-response-time.md) |

> **못 넘은 목표도 그대로 적습니다.** 접수가 느린 것은 **대기열이 찼을 때 문의를 버리지 않고
> 접수한 요청이 대신 분류를 떠안게 한 대가**입니다(D-045 ⑤) 
> 
> — *"느려지는 건 보이지만 사라지는 건 안 보인다"* 를 택한 결과이고, `PRD.md` §9 가 미리 예견한 트레이드오프입니다.

## 어떻게 도나

![접수부터 확정까지 — 문의 분류 파이프라인. 문의를 저장하고 즉시 202 를 응답한 뒤, 다른 스레드에서 캐시·DB 를 차례로 조회하고 필요할 때만 AI 를 호출한다. 확신도 미달 건과 자동 확정 중 20건에 1건 뽑힌 감사 표본이 검토 목록에 쌓이고, 상담원이 확정하면 캐시를 사람 답으로 덮어써 다음 문의부터 정정된 답이 나간다.](./presentation/assets/pipeline-architecture.png)
...
| **바꾸면 어떻게 되나 (계산값)** | **AI 가 답한 47건 기준** — `0.5` → 자동 확정 47건 · 사람이 볼 건 **0** / `0.8` → 32건 · 15건 / `0.9` → 16건 · 31건<br>⚠️ **여기에 못 읽은 3건은 안 들어 있습니다.** 그 3건은 기준값과 무관하게 **항상** 사람에게 가므로, 50건 전체로 세면 **3 / 18 / 34** 입니다 |
