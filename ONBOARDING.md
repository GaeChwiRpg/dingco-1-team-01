...
### 코드 말고, 사람이 해야 남은 일 하나 — 정답 50건의 2차 레이블

문의 본문 50건과 1차 정답은 다 채워졌다. 남은 것은 **두 번째 사람이 본문만 보고 정답을 따로 붙이는 일** 하나다. 이게 안 끝나면 측정 1 과 8ⓐ-1(이 프로젝트의 결론)이 읽을 데이터가 확정되지 않는다.

| | 어디를 보나 |
| --- | --- |
| 지금 상태와 하는 법 | `src/test/resources/seed/README.md` 「다 채운 뒤」 |
| 왜 두 사람이 따로 붙이나 | `GLOSSARY.md` 「2인 독립 레이블」 |
| 결과를 어디에 적나 | `evidence/seed-label-agreement.md` — 거기 있는 양식 그대로 |
| 누가 · 언제 | 이용택 또는 김은빈 · Day 1 (`PRD.md` §8 0단계, D-046 ⓓ) |

- **1차 정답(`inquiries-50.csv` 의 `expected_category`)을 보지 않고 붙인다.** 보고 붙이면 일치율이 실제보다 높게 나와서 재는 의미가 없어진다
- 갈린 건은 `PRD.md` §7 경계표로 판정하고, 표로도 안 갈리면 **표를 고친다.** 정답을 억지로 맞추지 않는다
- 일치율이 **90% 미만이면 코드보다 카테고리 경계를 먼저 고친다** (D-046)
