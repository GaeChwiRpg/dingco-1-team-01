...
### 실측 값 (2026-08-13, rows=100,000, 5회 실행 — 평균/최소/최대)
...
같이 쟀다. **평균만 남기면 실행별 편차가 큰지 알 수 없어**, 실행마다 걸린 시간을 따로 기록해
최소·최대도 함께 남긴다(코드리뷰 지적, PR #72).

| 방법 | 호출당 SQL 문 수 | 평균(ms) | 최소(ms) | 최대(ms) |
| --- | --- | --- | --- | --- |
| `@EntityGraph` (통째로 읽기) | 2회 | 636 | 436 | 862 |
| 프로젝션 (6칸만 읽기) | 2회 | 280 | 260 | 322 |
...
- **응답 시간은 프로젝션이 약 56% 빠르다**(636ms → 280ms, 평균 356ms 절약). `@EntityGraph`는
  큐·문의·분류결과 3개 테이블의 컬럼을 전부 엔티티로 매핑하고 지연 로딩 프록시까지 구성하는 반면,
  프로젝션은 응답에 실제 필요한 6칸만 바로 매핑한다 — 이 매핑·객체 생성 비용 차이로 보인다.
- **편차도 프로젝션이 훨씬 작다.** `@EntityGraph`는 최소~최대 폭이 426ms(436~862)로 넓은데,
  프로젝션은 62ms(260~322)로 좁다 — 프로젝션이 평균만 빠른 게 아니라 **실행마다 시간이 더
  일정하다.** `@EntityGraph`의 편차가 큰 이유는 이번 측정만으로는 단정하지 않는다(GC·엔티티
  매핑 비용의 변동으로 추정할 뿐, 별도로 확인하지 않았다).
- **채택**: 프로젝션(`searchProjected`)을 최종으로 쓴다. 쿼리 수는 같고 응답 시간·편차 둘 다
  확실히 줄어 손해 볼 이유가 없다. 다만 이 PR에서는 `search`를 지우지 않고 **`searchProjected`를
  controller/service 가 실제로 쓰도록 교체**하는 작업이 별도로 남는다 — 이 평가 자체는 "어느
  쪽이 빠른가"만 재는 것이 목적이라 교체는 후속 작업으로 분리한다.
- **한계**: 절대 시간은 이번 측정 환경(Windows + Docker Desktop, I/O가 느림)의 영향을 크게
  받아 운영 환경보다 부풀려져 있을 가능성이 높다. 실제로 같은 조건의 이전 실행에서는 평균이
  644ms/399ms(38% 차이)로, 이번 636ms/280ms(56% 차이)와 절대값·상대 차이 둘 다 달랐다 — **측정
  환경 자체의 실행 간 흔들림이 크다는 뜻이라, 절대 시간도 이번 상대 차이(56%)도 정확한 값으로
  못 박지 않는다.** "프로젝션이 EntityGraph보다 빠르고 더 안정적이다"라는 방향성만 결론의
  근거로 삼는다.
