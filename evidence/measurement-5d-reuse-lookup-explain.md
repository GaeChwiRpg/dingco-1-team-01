...
- **조인 인덱스 `idx_icr_inquiry_created` 는 현재 스키마에서 드롭이 실패한다** (실측 error 1553: "needed in a foreign key constraint"). `idx_icr_inquiry_created (inquiry_id, created_at DESC)` 가 `inquiry_id` 를 선행 컬럼으로 갖는 **유일한** 인덱스라 FK `fk_icr_inquiry(inquiry_id)` 요구를 단독으로 만족하기 때문이다. 별도로 `KEY (inquiry_id)` 를 두면 FK 를 유지한 채 이 복합 인덱스를 드롭할 수 있지만 — 현재 V2 스키마엔 그런 대체 인덱스가 없어, **지금 상태에서는** 이 인덱스를 뺄 수 없다.
- `Using temporary; Using filesort` 가 뜬다 — ORDER BY 가 `i.created_at`·`r.created_at`·`r.id` 로 **두 테이블에 걸쳐** 인덱스 하나로 정렬을 못 덮기 때문이다(D-037/D-041 이 수용한 구조). 정렬해야 하는 후보 행이 키당 판정 행 수(A 4건)뿐이라 이 규모에선 비용이 작다(0.107ms). **`LIMIT 1` 은 반환 행만 1건으로 제한할 뿐 정렬 입력을 줄이지 않는다** — 최상위 한 건을 고르려면 조건에 맞는 후보 행을 모두 읽고 필터·정렬한다.
...
- 판정 행을 200건까지 몰아도(hot 키) **0.363ms** 였다. 이 값은 **현재 데이터 분포에서의 실측**일 뿐이다 — A·B 두 점만으로 증가가 선형이라 단정하지 않는다(위와 같이 `LIMIT 1` 은 정렬 입력을 줄이지 않으므로 후보 행이 늘면 필터·정렬량이 는다). 지금 규모에서는 문제 될 값이 아니지만, **키당 수백 건 이상**의 목표 카디널리티에서는 다시 재본다.
