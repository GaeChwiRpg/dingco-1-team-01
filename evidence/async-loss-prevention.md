...
| 한계 | 내용 |
| --- | --- |
| ~~**운영 미적용**~~ | ~~"수정 후 0"은 테스트 전용 빈으로 시연한 것. 운영 `ClassificationService` 는 무변경(D-066 채택 대기)~~ → **이 한계는 없어졌다** (2026-08-14, TRI-96). 운영에 적용하고 애노테이션 경로로 다시 재서 유실 0 을 확인했다 — `async-loss-requires-new-adoption.md` |
...
| **커넥션 상한 안에서만** | `REQUIRES_NEW` 가 인라인마다 커넥션을 하나 더 쓰므로(심층 보고서 17절) 접수 스레드를 8(<풀 20)로 제한했다. 더 높은 동시성에선 풀 여유를 다시 봐야 한다<br>⚠️ **재봤고, 이 한계가 실재한다 (2026-08-14, TRI-96).** 접수 스레드를 24(>풀 20)로 올리면 커넥션이 말라 **유실이 0 이 아니다**(122~142). 같은 조건 `REQUIRED` 는 195 라 여전히 낫지만 완전 방어는 아니다 — `async-loss-requires-new-adoption.md` 확인 ⓑ |
...
>
> ✅ **예고한 대로 됐다 (2026-08-14, TRI-96).** D-066 이 채택돼 운영이 `REQUIRES_NEW` 가 되면서
> 이 테스트는 **`REQUIRED` 를 강제한 대조군 빈**으로 옮겨졌고, 유실 0 쪽이 운영 빈을 쓰게 됐다.
> **위 「실측」 표의 값은 그대로 유효하다** — 채택 후 같은 조건에서 다시 재도 177 → 0 이었다.
> 채택 실측과 **새로 발견된 커넥션 한계**는 `evidence/async-loss-requires-new-adoption.md` (김준현).
