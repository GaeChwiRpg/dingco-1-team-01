...
### ★ 이 표가 말하는 것 — "검토가능"과 "stuck"은 같은 입력에서 동시에 나오지 않는다

카드 Result gate 는 *"원문 유실 0 · 검토가능 전환 · stuckReceived 관측을 **같은 실패 입력으로**
확인하라"* 고 했다. 실제로 재보면 **한 입력에서 셋이 동시에 나올 수 없다** — 이게 이 카드가 판단할
지점이었다.

- 검토가능(큐에 있음)이면 그 건은 **stuck 이 아니다**(분류가 완주해 사람에게 넘어갔다).
- stuck(RECEIVED 방치)이면 그 건은 **큐에 없다**(분류가 못 끝났다).

즉 **"검토가능"과 "stuck"은 서로 다른 실패의 결과**다. 그래서 "같은 실패 입력으로 셋 확인"은
문자 그대로 성립하지 않고, 올바른 형태는 **"하나의 일관된 실험 장치로 taxonomy 를 훑어 각 모드의
결과를 예측대로 확인"** 하는 것이다. 이 표가 그 형태다.
