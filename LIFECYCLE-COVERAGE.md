...
> **기준 시점: 2026-08-05** (PR #1~#7 머지 / PR #8 리뷰 중 / **도메인 전환 PR 작업 중 — D-031**).
>
...
| 단계 | 책임자 | 핵심 도구 | 산출물 | 상태 |
| --- | --- | --- | --- | --- |
| 1. 기획 | 김준현 | Jira MCP, AI PRD | `PRD.md`, `DECISIONS.md` | ✅ Phase 2 완료 (D-001~D-031) — **D-031 으로 도메인 전환, PRD 전면 재작성** |
| 2. 코딩 | 팀 전원 | claude.md, Commands, Hooks, gh CLI | `CLAUDE.md`, `API-CONTRACT.md`, `src/`, `.claude/` | 🔄 baseline **재작성** (D-031) — 엔티티 3종·enum·Flyway V1·정적 팩토리. Hooks 완료 (PR #7), 서브에이전트 5종은 **PR #8 리뷰 중**. `service/`·`api/` 미착수, `.claude/commands/` 미작성 |
...
- 산출물: `PRD.md`(12절 · US 13개 · 측정 11개), `DECISIONS.md`(D-001~D-031)
- 도구:
  ...
  - **도메인 전환도 같은 방식으로 처리했다 (D-031)** — 결정 15건이 폐기·개정됐지만 **본문은 하나도 지우지 않고 상태 필드만 갱신**했다. 그 결과 "무엇을 포기하고 이 도메인으로 왔는가"가 로그에 남는다. 전환 근거를 D-027 이 세운 판별 기준으로 평가한 것도 같은 취지다 — **기준이 있으니 탈락 사실과 그 대가를 계산할 수 있었다**
- 한계:
...
- 산출물:
  ...
  - `src/test/java/` — `BaselineSmokeTest`(Testcontainers), `DomainFactoryTest`(순수 단위). `CandidateIndexMigrationTest` 는 **V2 후보 인덱스와 함께 삭제**됐다 (D-031)
...
- 산출물:
  ...
  - `MONITORING.md` — ⚠️ **아직 템플릿 상태다.** 파일은 있으나 도구 선택 표가 미기입이다
- 도구: Docker Compose (동작). Sentry MCP (미연동)
- 관측 설계는 되어 있다 — `GET /api/stats` 5개 블록 + Actuator gauge 6종(`triage.inquiries.stuck_received` 포함). **노출할 코드가 없을 뿐 무엇을 볼지는 정해져 있다** (`API-CONTRACT.md` §7~§8)
- 한계:
  - **실 트래픽 0.** 부하 측정(§5-a, §5-b)과 `stuckReceived` 실측은 `service/`·`api/` 착수 후
  - **Sentry MCP 미연동** — 잔여 4건 중 1건. **도메인 전환으로 역할 중복이 오히려 해소됐다** — 이전에는 자체 에러 수집 파이프라인이라 Sentry 와 겹쳤지만, 지금은 이 시스템이 **CS 문의**를 다루고 Sentry 는 **앱 자체의 장애**를 본다. 경계가 분명해져 시연 시나리오를 잡기 쉬워졌다
...
| --- | --- | --- | --- |
| 기획 | ✅ PRD.md, DECISIONS.md (D-001~D-031) | ⏳ Jira MCP dry-run 미시연 | 🔄 |
| 코딩 | ✅ src/ (엔티티 3종·enum·repository·Flyway V1·정적 팩토리), CLAUDE.md, API-CONTRACT.md **v1.0** | 🔄 claude.md ✅ / Hooks ✅ / 서브에이전트 5종 🔄(PR #8 리뷰 중) / **Commands 미작성** | 🔄 |
| 테스트 | ✅ tests/e2e/, e2e.yml, Testcontainers 테스트 | ⏳ Playwright MCP 시나리오는 health 1건뿐 | 🔄 |
