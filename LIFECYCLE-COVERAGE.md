...
| 3 | **롤백 경계 검증** — ② 롤백 + ① **생존** 동시 확인 + `stuckReceived` 증가 (D-031) + **감사 삽입을 `REQUIRES_NEW` 로 뺀 대조 — 표본이 몇 건 새는지** (D-045 ②) | 이용택 (P1) | **Day 3 필수 체크포인트** |
...
> **측정 3·11 도 이용택(P1) 이 맡는다 (2026-08-06 정정).** `stuckReceived` gauge 와 캐시 레이어(1단·2단 절감 경로)는 위 「코딩 단계 작업 패키지」 표에 적힌 대로 D-031 재배정 이후 **P1 소유**다 — 만든 사람이 지표가 0 인 것이 "정상"인지 "안 세고 있는 것"인지 구분할 수 있어야 자기 패키지 원칙이 성립한다. **이전 판은 김은빈(P3) 으로 적어뒀는데, 재배정 전 P1/P3 경계를 그대로 옮겨 적은 오기였다** — Jira 실제 배정(`TRI-71`·`TRI-72`·`TRI-73`)과 대조해 정정했다. **Day 는 그대로 Day 3 이다** — Jira 에는 실행 요일 필드가 없고, `PROJECT-BRIEF.md`(Line 185·199)가 이미 Day 3 을 「필수 체크포인트」로 못박은 설계 의도(② 롤백을 다른 Day 4 측정보다 먼저 검증해 늦은 발견을 막는다)가 있어 담당자만 바꾸고 시점은 그대로 뒀다. ⚠️ **PM 구현순서 메시지(TRI-72·73 을 Day 4 로 배치)와는 다르다** — 그 메시지를 따르려면 이 문서와 `PROJECT-BRIEF.md` 를 함께 Day 4 로 옮기는 별도 결정이 필요하고, 그러면 체크포인트가 다른 Day 4 측정과 동시에 걸려 늦은 발견을 막으려던 원래 취지가 약해진다.
...
> 측정 1·4·12 가 김준현에게 몰린 것은 **AI 호출·재시도·정답 레이블이 전부 P2 소유**이기 때문이다. 다만 Day 4 에 겹치므로 측정 4 를 Day 3(측정 3 과 같은 날)으로 당겨 분산한다.
