...
| 2. 코딩 | 팀 전원 | claude.md, Commands, Hooks, gh CLI | `CLAUDE.md`, `API-CONTRACT.md`, `src/`, `.claude/` | 🔄 baseline **재작성** (D-031) — 엔티티 3종·enum·Flyway V1/V2·정적 팩토리. Hooks 완료 (PR #7), 서브에이전트 5종 완료 (PR #8). **`config/` 완료(TRI-25/26)** — 헤더 인증 필터 + endpoint 별 역할 매핑. **`api/` 착수(TRI-27~30, TRI-32)** — 공용 예외 처리 지점 + 오류 DTO + Sentry 500 배선, 응답 6종 전수 실측 / `POST /api/inquiries`. **`service/` 착수(2026-08-06~07)** — P1 `ContentMasker`·`NormalizedKeyGenerator`·트랜잭션 ①(TRI-31·33·37~39) / P2 `service/ai` AI 호출·응답 파싱·값 검증 4종(TRI-46·48·49·50). **Lombok 도입 — `@Data`·`@Setter` 는 컴파일 에러로 차단**(D-057). 테스트 **88건** 통과. ✅ **`.claude/commands/ticket.md` 작성 완료(PR #33)** — Jira 실시간 조회로 티켓 컨텍스트·추천·팀 현황판·상태 동기화. 실행하다 규칙 결함을 발견해 예외 2개를 못박음(사례 18). ✅ **1단·2단 절감 경로 완료(TRI-40·41·84, PR #42 머지)** — 계약 C 값 구조 + 재사용 조회(1순위 사람 답 / 2순위 AI) + 원자적 put(사람 답 보호) + 메모리 상한(allkeys-lru). **잔여: 통계·측정** |
...
> | 패키지 | 끝난 것 | 다음 |
> | --- | --- | --- |
> | **P1** | `ContentMasker`·`NormalizedKeyGenerator`(D-052·D-053) · 트랜잭션 ① · `POST /api/inquiries` · 계약 A 발행 · 목록·상세 조회 · **1단 캐시 + 2단 절감 경로**(TRI-40·41·84, PR #42) | **EXPLAIN 실측(TRI-42) · 과도/과소 병합 테스트(TRI-43)** |
