...
| 2. 코딩 | 팀 전원 | claude.md, Commands, Hooks, gh CLI | `CLAUDE.md`, `API-CONTRACT.md`, `src/`, `.claude/` | 🔄 baseline **재작성** (D-030) — 엔티티 3종·enum·Flyway V1·정적 팩토리. Hooks 완료 (PR #7), 서브에이전트 5종은 **PR #8 리뷰 중**. `service/`·`api/` 미착수, `.claude/commands/` 미작성 |
...
| 패키지 | 담당 | 범위 | 착수 조건 |
| --- | --- | --- | --- |
| **P1** 접수·절감 경로 | 이용택 | `POST /api/inquiries`, **정규화 규칙 + 마스킹**, 트랜잭션 ①, 계약 A 발행, **2단 절감 경로 + 캐시 레이어**, 재사용 조회 우선순위(D-032) | **없음 — baseline PR 도 본인 담당이므로 즉시 착수** |
...
| 8ⓑ | **재사용 건의 오분류율** — ⓐ 와 섞지 않는다. 비교할 AI 답이 없기 때문 (D-032) | 김준현 (P2) | Day 4 |
...
- 산출물: `PRD.md`(12절 · US 13개 · 측정 11개), `DECISIONS.md`(D-001~D-030)
- 도구:
