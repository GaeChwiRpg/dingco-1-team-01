...
| AI 분류 확신도가 임계값 미만 → `NEEDS_REVIEW` 로 격리 (정상 흐름, 예외 아님) | **모른다 — 그리고 몰라도 된다** | 이건 애초에 Sentry 의 대상이 아니다. 이 프로젝트의 **검토 큐**가 담당하는 "비즈니스 판단 실패"이지 Sentry 가 다루는 "애플리케이션 실패"가 아니다 (아래 3번 참조) |
...
| 트리거 | 예외 발생 (자동) 또는 명시적 호출 | 확신도가 임계값 미달, 감사 표본 추출 — 예외 없음 |
...
| 자리 | 언제 | 코드상 | 실측 |
| --- | --- | --- | --- |
| `RetryingAiClassifier.recoverFromCallFailure` | AI 를 **끝내 못 부른** 최종 실패 | 보냄 | ✅ **실트래픽 확인**(2026-08-10, 측정 4) |
| `RetryingAiClassifier.recoverFromInvalidResponse` | AI 답을 **끝내 못 읽은** 최종 실패 | ❌ **일부러 안 보냄** | — |
| `GlobalExceptionHandler` | API 요청에서 처리 못 한 오류(500) | 보냄 | ⬜ **미검증** — 아래 참조 |
| `AsyncConfig` (비동기 예외 핸들러) | 분류 스레드에서 아무도 안 잡은 예외 | 보냄 | ⬜ 미검증 |

**「코드상」과 「실측」을 나눈 이유** — 캡처 호출이 코드에 있다는 것과 그것이 실제 트래픽에서 도착한다는 것은 다른 말이다. 지금 **실트래픽으로 확인된 것은 첫 줄 하나뿐**이다.

⚠️ **`api/` 500 경로를 「검증됨」으로 읽지 않는다.** 2-5 에서 자동 캡처를 실측했지만 그건 **지금 저장소에 없는 임시 컨트롤러**(`SentryMvcVerificationController`, 검증 후 삭제)로 확인한 것이고, `Spring6ExceptionResolver` 라는 **자동 경로**였다. 지금 `api/` 에 있는 `GlobalExceptionHandler` 는 **예외를 잡아서 직접 캡처를 부르는 수동 경로**라 메커니즘이 다르다 — 실제 500 요청으로는 아직 확인하지 않았다.
...
**⚠️ 비율의 분모를 헷갈리지 않게 적어둔다.** 셋은 서로 다른 것을 세고 있다.

| 값 | 분자 | 분모 | 정하는 것 |
| --- | --- | --- | --- |
| 오류 이벤트 **100%** | Sentry 로 간 이벤트 | **위 표에서 「보냄」인 자리에 도달한 예외** | 샘플링 없음 (`sentry.sample-rate` 미설정 = 기본 1.0) |
| 성능 추적 **10%** | 전송된 트랜잭션 | **처리한 HTTP 요청 전체** (성공·실패 무관) | `traces-sample-rate: 0.1` |
| 로그 **0%** | — | 앱이 남긴 모든 로그 | `sentry-logback` 의존성이 없음 |

**「예외 전체의 100%」가 아니다** — 위 표에서 「일부러 안 보냄」인 자리에 도달한 예외는 애초에 분자에도 분모에도 안 들어간다. 「보낸다」로 정한 것 중에서 유실 없이 100% 라는 뜻이다.

**「3회 시도 → 이벤트 1건」의 분모도 요청이 아니다** — 문의 1건을 처리하는 동안 AI 호출이 3번 일어났고 그중 캡처 지점(`@Recover`)에 도달한 것이 1번이라는 뜻이다 (3-0-2 참조).

### 3-0-1. 이 프로젝트의 관측 장치는 네 층으로 나뉘어 있다

```text
애플리케이션이 고장났나        → Sentry              (개발자가 코드를 고친다)
분류가 멈춰 있나                → stuckReceived       (0 이 아니면 ②가 완주 못 한 건이 있다)
