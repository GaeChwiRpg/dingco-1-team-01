...
| a·b | ✅ **2026-08-13 실측 (PR #77)** — **둘 다 목표 미달.** a 6.36~18.77초(목표 0.1) / b 7.59초(목표 0.2) | `api-response-time.md` |
| c | ✅ **절감률 49.0%** (측정 6, PR #76) | `measurement-6-11-actual.md` |

> **8/13 에 2 · 6 · 11 · a·b·c 가 전부 채워졌다.** 남은 것은 **5 의 일부 케이스와 10(blind 재점검)** 뿐이다. 나머지 조건과 한계는 각 evidence 문서 맨 위에 적혀 있다.
...
접수한 쪽이 대신 처리한다(D-045 ⑤). 그래서 **AI 키 없이 잰 회차에서** 요청의 **73~74% 는 1초
안에 끝나고, 나머지 26~27% 만 6~9초대**로 갈렸다 — 평균을 내면 이 사실이 사라진다.
**실제 키를 넣은 회차는 느린 쪽이 16.94~18.77초로 더 커진다** — 갈리는 비율이 아니라 **느린
쪽의 크기**가 조건에 따라 달라진다.
...
> **어느 한쪽도 틀리지 않았는데 조합이 틀렸다.** 상세는 `evidence/measurement-6-11-actual.md`, 수정은 **TRI-96**.
...
| 멈춘 문의 자동 재분류 | **없음** (「나중에 할 것」 E) |
| 검토 항목 선점 | **있음** (TRI-93, PR #86, `DECISIONS.md` D-067 — 「나중에 할 것」 H 였던 것이 완료됨) |
...
| --- | --- | --- |
| P — 문제 정의 | 됨 | — |
| A — 대안 비교 | 됨 (`DECISIONS.md` 67건) | **여기에 더 쓰지 않는다** |
...
> **§9 「속도 목표」 아래 경고 블록 참조** — 소관은 P2, 티켓은 **TRI-96**.
