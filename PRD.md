...
**아직 아무것도 재지 않았다.** 무엇을 어떻게 잴지는 §9에 있고, 그중 결론에 해당하는 것은 셋이다.

| 무엇 | 어떻게 읽나 |
| --- | --- |
| **8ⓐ-1 · 자동 확정 건의 실제 오분류율** (확신도 구간별) | 이 숫자가 0이 아니면 **옵션 C로는 못 잡는 실패가 실재한다**는 증거다 |
| **8ⓐ-2 · 5% 감사가 그중 몇 건을 집어냈나** | 8ⓐ-1과의 차이가 곧 **5% 샘플링의 검출 한계**다. 한계를 아는 것까지가 설계다 |
| **12 · 상담원 2명의 분류가 얼마나 일치하나** | 8ⓐ의 오분류율 중 "사람끼리도 갈리는 몫"을 떼어내는 값. 이게 없으면 8ⓐ를 해석할 수 없다 |
...
| | 내용 | 왜 미루나 |
| --- | --- | --- |
| **H** | **상담원이 검토 항목을 열면 "내가 처리 중"으로 잡아두기** | 지금은 두 상담원이 같은 문의를 동시에 붙들고 각자 분류한 뒤, 확정할 때가 되어서야 한 명이 409를 받는다. 늦게 누른 쪽의 작업이 통째로 버려진다. **미룬 것 중 가장 먼저 붙인다** (D-032) |
...
| --- | --- | --- |
| P — 문제 정의 | 됨 | — |
| A — 대안 비교 | 됨 (`DECISIONS.md` 42건) | **여기에 더 쓰지 않는다.** 새 설계 논쟁은 코드가 막힐 때만 |
| A — 구현 | **거의 없음** | §8의 1단계 7개 |
| R — 측정 | **없음** | §9의 12개. 1단계가 끝나야 첫 숫자가 나온다 |

**A(대안 비교)에 쓰는 시간을 지금부터 0에 가깝게 줄인다.** 설계 논쟁은 이미 충분하고, 여기서 더 늘려도 §11의 성공 기준 중 어느 것도 채워지지 않는다.
...
| 3 | 그 숫자가 실제보다 낮다는 것까지 | 측정 8ⓐ-2 = 0.333(감사가 3건 중 1건만 잡음) + **측정 12 = 경계 20건에서 16/20 일치**(사람끼리 갈리는 몫) + AI 답을 보여줘서 생기는 쏠림 | ✅ |
...
| **정답지를 만들 때의 쏠림** | 관측값을 **낮춘다** | **−1건** — 2차 blind 답 기준으로 채점하면 오히려 낮다 |
...
> ⚠️ **측정 12 는 경계 사례를 일부러 많이 넣은 20건이라 대표 표본이 아니다.** 그 20건의 일치율 80.0% 를 모집단 수치로 쓰지 않는다 — 모집단 인용값은 전체 50건의 **90.0%**(D-046)이고, 이번 20건은 *"경계에서 사람이 실제로 갈리는가"* 를 보여주는 보강 증거다.
>
