 ...
 * JSON 직렬화에도 새어 들어간다.
 *
 * <p><b>캐시 이름마다 담는 타입을 미리 못박는다 — 「무엇이든 담을 수 있는 캐시」로 만들지 않는다.</b>
 * {@code stats:summary:backlog} 에는 {@link StatsService.Backlog} 만, {@code stats:summary:audit}
 * 에는 {@link StatsService.Audit} 만 들어간다는 것을 <b>이미 알고 있으므로</b>, 값 안에 타입 이름을
 * 적어 두는 방식(다형 타이핑)이 아니라 <b>타입을 지정한 직렬화기</b>
 * ({@link Jackson2JsonRedisSerializer})를 캐시 이름마다 하나씩 쓴다.
 *
 * <p>이 선택은 아래 두 문제를 <b>동시에</b> 없앤다 — 둘 다 실제로 재현했다(TRI-68 테스트 중 발견).
 *
 * <ol>
 *   <li><b>꺼낼 때 엉뚱한 타입이 되는 것.</b> 타입 정보 없이 담으면 record 가
 *       {@code LinkedHashMap} 으로 복원돼 {@code ClassCastException} 으로 500 이 난다 — 캐시를
 *       쓰는 이유가 "반복 요청에 다시 계산 안 하기"인데 <b>정확히 그 반복 요청(TTL 안의 재조회)에서
 *       터진다.</b> {@code DefaultTyping.NON_FINAL} 로도 안 고쳐진다(실제로 시도해 재현했다) — 그
 *       정책은 선언 타입이 아니라 <b>값의 실제(런타임) 클래스가 final 인가</b>로 타이핑 여부를
 *       정하는데, record 는 전부 final 이라 애초에 안 붙는다. 타입을 지정한 직렬화기는 담을 때가
 *       아니라 <b>꺼낼 때 그 타입으로 읽으므로</b> 이 문제 자체가 생기지 않는다
 ...
 * {@code StatsService} 중첩 타입으로만 좁히면 값 안의 {@code EnumMap}·{@code List}·{@code Long}
 * 까지 거부돼 <b>두 번째 조회가 항상 실패한다.</b> 그렇다고 {@code java.util}·{@code java.lang} 을
 * 열면 좁힌 의미가 옅어진다. 담는 타입이 캐시 이름마다 하나로 정해져 있으면 다형 타이핑 자체가
 * 필요 없는 장치다.
 *
 * <p><b>담는 타입이 또 늘어나면</b>(계약 §7 의 {@code cache} 블록) 캐시 이름을 하나 더 나눠 전용
 * 직렬화기를 준다. 캐시 이름 하나에 여러 타입을 섞기 시작하면 다시 다형 타이핑이 필요해지고,
 * 그 순간 위 2번(CWE-502)이 돌아온다.
 */
...
public class CacheConfig {
    ...
    RedisCacheManager cacheManager(RedisConnectionFactory connectionFactory, ObjectMapper objectMapper) {
        ...
        return RedisCacheManager.builder(connectionFactory)
                .withCacheConfiguration(BACKLOG_CACHE, typedConfig(cacheObjectMapper, StatsService.Backlog.class))
    ...
    private static <T> RedisCacheConfiguration typedConfig(ObjectMapper cacheObjectMapper, Class<T> type) {
        RedisSerializer<T> serializer = new Jackson2JsonRedisSerializer<>(cacheObjectMapper, type);
