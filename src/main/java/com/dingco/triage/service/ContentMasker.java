...
public class ContentMasker {
     ...
     * 제거</b>하므로, 마커로 감싼 키 토큰(예: {@code U+E000 ORDER U+E000})은 <b>이 클래스의
     * 마스킹만 만들 수 있다 — 어떤 자연어 입력도 그 토큰을 재현(위조)할 수 없다.</b> 이것이
     * 실제로 필요한 보증이다: 서로 다른 문의가 같은 키가 되는 과도 병합을 막는다.
     *
     * <p><b>「단사(injective)」가 아니다 — 정확히는 「토큰 위조 불가」다 (CodeRabbit 리뷰).</b>
     * 입력에서 마커를 지우는 것은 손실이라, {@code "문의"} 와 (사용자가 못 치는) {@code "문<U+E000>의"}
     * 는 같은 키가 된다. 이 손실은 <b>수용한다</b> — 실제 문의엔 U+E000 이 들어오지 않으므로
     * 현실 트래픽의 과도 병합을 만들지 않는다. 엄밀한 단사가 필요하면 마커 이스케이프(2배화)를
     * 쓰면 되지만, 현실에 없는 입력만 막자고 복잡도를 얹지 않는다(D-018 계보).
     */
     ...
     *
     * <p><b>토큰 위조 불가 보장</b> — 마스킹 전에 입력에서 {@link #KEY_MARK} 를 먼저 제거한다.
     * 그러면 마커로 감싼 키 토큰은 이 마스킹만 만들 수 있어, 어떤 자연어 입력도(심지어 마커를
     * 직접 붙여넣어도) 그 토큰을 재현할 수 없다 — 서로 다른 문의가 같은 키가 되는 과도 병합을
     * 구조적으로 차단한다. (엄밀한 단사는 아니다 — {@link #KEY_MARK} 참조.)
     *
