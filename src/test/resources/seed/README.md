...
**레이블 → 대조 → 기록 세 단계가 끝났다.** 일치율 **98.0%** (하한 90.0% — 조건의 한계는 `evidence/seed-label-agreement.md` 참조).
**남은 것은 갈린 1건의 정답 확정 하나뿐이고, 그게 끝나야 `expected_category` 를 사람 답으로 교체할 수 있다.**
...
| **4** | **갈린 1건(`id 24` · ref 45)의 정답 확정** | **두 사람** | ⬜ §7 표로는 `ACCOUNT`(「탈퇴」 명시). **2차 작성자 확인 대기** |
| **5** | **`expected_category` 를 확정된 사람 답으로 교체** | 김준현 | ⬜ 4번이 끝난 뒤. **이게 끝나야 TRI-77(측정 1·8ⓐ)을 시작할 수 있다** |
| — | 2차 작성자에게 줄 blind 서식 만들기 | 김준현 | ✅ `inquiries-50-blind.csv` — 답·힌트·**원본 `id`** 빼고 순서 섞음 |
| — | `content` 50건 (경계 9건 포함) | 사람 | ✅ 채워짐 |
| — | `expected_category` | **AI** | ⚠️ **아직 AI 가 붙인 값이다.** 사람 답 50건은 나왔지만 **교체 전이다** (위 5번) |
| — | `channel` | 아무나 | ✅ 채워짐 |
