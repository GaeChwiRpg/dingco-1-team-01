...
| 5 | **`expected_category` 를 확정된 사람 답으로 교체** | 김준현 | ✅ **7건 교체.** 분포가 10종×5건에서 벗어났지만 **맞추지 않았다** (evidence 참조) |
| — | 2차 작성자에게 줄 blind 서식 만들기 | 김준현 | ✅ `inquiries-50-blind.csv` — 답·힌트·**원본 `id`** 빼고 순서 섞음 |
| — | `content` 50건 (경계 9건 포함) | 사람 | ✅ 채워짐 |
| — | `expected_category` | **사람** | ✅ **두 사람이 붙인 확정답으로 교체됨** (2026-08-10). AI 초안은 `git log` 로만 남는다 |
| — | `channel` | 아무나 | ✅ 채워짐 |
