...
| 2차 작성자에게 줄 blind 파일 만들기 | 김준현 (1차 작성자) | ✅ `inquiries-50-blind.csv` — 답·힌트·**원본 `id`** 빼고 순서 섞음 |
...
# 정답 문자열이 blind 파일에 새지 않았는지 (0 이 나와야 한다)
grep -c -E 'DELIVERY|RETURN_REFUND|PAYMENT|PRODUCT|ACCOUNT|ORDER_CHANGE|PROMOTION|SERVICE_USAGE|COMPLAINT|ETC' \
