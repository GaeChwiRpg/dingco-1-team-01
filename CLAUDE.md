...
**이 도메인이라야 하는 이유 — 판별 기준 3개 (D-027)**

설계 자산의 대부분은 도메인 중립이라 도메인이 교체 가능해 보이지만, 아래 3개를 못 넘는 도메인에서는 이 설계가 성립하지 않는다. **도메인 교체 제안은 세 기준 통과를 먼저 보인다.** 못 넘으면 새 `DECISIONS.md` 항목 없이 반려한다.

1. **묶음 동질성** — 묶인 N건이 같은 판정을 받는가. 안 맞으면 그룹핑이 정보를 파괴한다 (D-004 의 전제)
2. **정답 단일성** — 팀이 확정한 정답이 **유일한가**. 안 맞으면 오분류율에 검토자 불일치가 섞여 측정 8 을 읽을 수 없다 (측정 1·8·9 의 전제)
3. **AI 존재 이유** — 규칙·룩업 테이블로 풀리지 않는가. 안 맞으면 AI 가 부수 기능으로 강등된다 (D-001 의 전제)

> 판정 대상이 **사람·거래·개인 건**이면 기준 1 에서 탈락하고, **패턴·시그니처·콘텐츠**면 통과한다. 검토·탈락한 후보 4종은 D-027 참조.
